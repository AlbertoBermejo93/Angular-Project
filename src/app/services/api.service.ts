...
export class ApiService {
  ...
  getBeer(id: number): any{
    return this.bieres.find((element) => element.id == id);
  }

  getBeers(limit : number){
    return this.bieres.slice(0,limit);
  }
  
  //Retourne un livre dont l'ID est passé en parametre
  getBiere(parameter : number){
    return this.bieres.find((element) => element.id == parameter)
